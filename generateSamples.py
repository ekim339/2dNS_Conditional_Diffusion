...
def generate_and_plot_sample(
    ...
    """
    Generate multiple samples from a true field, average them pixel-wise, and return original, averaged generated, and difference.
    
    Args:
        num_samples: Number of samples to generate and average (default: 10)
    
    Returns:
        x_true: Original field (numpy array)
        x_pred: Averaged generated field (numpy array)
    ...
    with torch.no_grad():
        for i in range(num_samples):
            ...
            # Unnormalize each sample
            if x_pred_norm.dim() == 4:
                x_pred_norm = x_pred_norm.squeeze()
            x_pred = x_pred_norm.cpu() * (std + 1e-8) + mean  # (64, 64)
            samples_list.append(x_pred)
    
    # Stack all samples and average pixel-wise
    samples_tensor = torch.stack(samples_list, dim=0)  # (num_samples, 64, 64)
    x_pred_avg = samples_tensor.mean(dim=0)  # (64, 64) - pixel-wise average
    
    print(f"Done. Averaging across {num_samples} samples.")
    
    x_true = x0_true.cpu()  # (64, 64)
    
    # Convert to numpy
    x_true = x_true.numpy()
    x_pred = x_pred_avg.numpy()
    diff = x_true - x_pred
...
def compare_train_and_test_samples(
    ...
    """
    Generate samples from first train sample and first test sample, then plot comparisons.
    """
    ...
    print(f"\n{'='*60}")
    print(f"Processing Train Sample (index 50000)")
    print(f"{'='*60}")
    train_sample = train_full[50000]
    x_true_train, x_pred_train, diff_train, mse_train, mae_train = generate_and_plot_sample(
        trainer, train_sample, mean, std, device, guidance_scale, "Train", num_samples=10
    )
    print(f"Train Sample (averaged over 10 samples) MSE: {mse_train:.6f}, MAE: {mae_train:.6f}")
    ...
    x_true_test, x_pred_test, diff_test, mse_test, mae_test = generate_and_plot_sample(
        trainer, test_sample, mean, std, device, guidance_scale, "Test", num_samples=10
    )
    print(f"Test Sample (averaged over 10 samples) MSE: {mse_test:.6f}, MAE: {mae_test:.6f}")
    ...
    print(f"Summary")
    print(f"{'='*60}")
    print(f"Train Sample (index 50000, averaged over 10 samples):")
    print(f"  MSE: {mse_train:.6f}")
    print(f"  MAE: {mae_train:.6f}")
    print(f"\nTest Sample (index 0 in test set, averaged over 10 samples):")
