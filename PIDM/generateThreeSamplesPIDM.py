"""
Visualize PIDM (physics-informed conditional DDPM) reconstructions.

Same layout as ddpm_sparse_cfg/generateThreeSamples.py, but loads
`PIDM/src/cfgConditional.py`: sparse 8×8 conditioning, checkpoint from PIDM training
(physics loss only affects training; inference is CFG sampling like the plain script).

Saves two 3×3 figures under PIDM/samples/: train and test.
"""
import os
import sys
...
# PIDM/src on path for cfgConditional
_PIDM_ROOT = Path(__file__).resolve().parent
_SRC = _PIDM_ROOT / "src"
sys.path.insert(0, str(_SRC))

from cfgConditional import (  # noqa: E402
    ConditionalDDPM,
    DDPMTrainer,
    DiffusionConfig,
    default_device,
...
def generate_and_plot_sample(
    ...
    """
    One CFG sample from sparse 8×8 observations vs ground truth (PIDM architecture).
    """
    if x0_true.dim() > 2:
    ...
    coords = torch.arange(0, 64, sensor_stride, dtype=torch.long)
    assert len(coords) == 64 // sensor_stride, "Check sensor_stride vs 64."
    c = coords
    y_sparse = x0_true_norm[c][:, c]
    assert y_sparse.shape[0] == y_sparse.shape[1] == 64 // sensor_stride

    y_input = y_sparse.unsqueeze(0).unsqueeze(0).to(device)
...
def _plot_split_grid(rows, split_name: str, save_path: str):
    ...
    fig.suptitle(
        f"{split_name.capitalize()} — PIDM: original vs generated vs difference",
        fontsize=13,
        y=1.002,
    )
    plt.tight_layout()
...
def compare_train_and_test_samples(
    ...
    print(f"Total samples: {N}, train: {n_train}, test: {len(test_full)}")

    print(f"\nLoading PIDM checkpoint from: {ckpt_path}")
    try:
    ...
    if samples_dir is None:
        samples_dir = str(_PIDM_ROOT / "samples")
    ...
    print(f"\n{'='*60}\nSummary ({len(all_rows)} samples) — PIDM\n{'='*60}")
    for row in all_rows:
        print(
