"""Evaluate conditional DDPM reconstructions and their Kolmogorov-flow residual.

For every selected center frame, the model independently predicts the five
consecutive vorticity fields from k-2 through k+2 from their sparse observations.
The physics residual is computed only from those predictions; ground-truth
fields are used only for reconstruction metrics.

The spatial right-hand side is a differentiable PyTorch port of the operator
used by evaluate_continuous_2dns_kolmogorov.py and
...
except ImportError:  # Direct execution from ddpm_sparse_cfg/.
    from cfgConditional import (
        ConditionalDDPM,
        DDPMTrainer,
        DiffusionConfig,
...
class KolmogorovResidual:
    """Repository-consistent residual for five consecutive predicted frames."""
    ...
    def _operators(
        ...
        # FlowConfig.create_mesh() uses endpoint-inclusive linspace arrays for
        # the repository's sampled velocity forcing f=[sin(k*y), 0].
    ...
    def repository_rhs(self, omega: torch.Tensor) -> torch.Tensor:
        ...
        # utils.dealiasing() returns the original JAX array because its .at[].set
...
class FiveFrameSparseEvaluationDataset(Dataset):
    """Center targets and sparse conditions for k-2 through k+2."""
    ...
    def __init__(
        ...
        if full_fields.ndim != 3 or full_fields.shape[1:] != GRID_SIZE:
            raise ValueError(
                f"Expected full_fields with shape (N,64,64), got {full_fields.shape}"
            )
        if sensor_stride <= 0:
            raise ValueError(
                f"sensor_stride must be positive, got {sensor_stride}"
            )
    ...
    def _sparse_condition(self, field_norm: torch.Tensor) -> torch.Tensor:
        mask = torch.zeros_like(field_norm)
        mask[:: self.sensor_stride, :: self.sensor_stride] = 1.0
        return torch.stack((field_norm * mask, mask), dim=0)

    def __getitem__(self, index: int) -> Tuple[torch.Tensor, torch.Tensor]:
        center = index + 2
        normalized_fields = [
            self._normalize(self.x[frame_index])
            for frame_index in range(center - 2, center + 3)
        ]
        conditions = torch.stack(
            [self._sparse_condition(field) for field in normalized_fields],
            dim=0,
        )
        return normalized_fields[2].unsqueeze(0), conditions
...
def _sample_subset(
    ...
    """Sample center indices without breaking five-frame temporal windows."""
    if count <= 0:
...
def evaluate_on_test(
    ...
    """Sample predicted five-frame windows and report evaluation metrics."""
    ...
    print("Residual input: five model-predicted consecutive frames")
    ...
    for batch_index, batch in enumerate(data_loader):
        ...
        if len(batch) != 2:
            raise ValueError(
                "Expected (center_field, five_conditions), got "
                f"{len(batch)} tensors"
            )

        omega_norm, conditions = batch
        if conditions.ndim != 5 or conditions.shape[1:3] != (5, 2):
            raise ValueError(
                "Expected conditions with shape (B,5,2,64,64), got "
        ...
        predicted_normalized_fields = [
            trainer.sample_cfg(
                cond=conditions[:, offset],
        ...
        predicted_fields = [
            field * scale + mean for field in predicted_normalized_fields
        ]
        omega_pred = predicted_fields[2]

        # No ground-truth field participates in this residual.
        residual = residual_evaluator(
            *(field.float() for field in predicted_fields)
        )

        field_error = omega_pred - omega_true
        mask = conditions[:, 2, 1:2]
...
def run_eval(
    ...
    """Evaluate randomly selected, temporally intact five-frame windows."""
    ...
    # Kolmogorov dataset while still exposing a writable NumPy buffer to Torch.
    ...
    train_windows = FiveFrameSparseEvaluationDataset(
        train_full, mean=mean, std=std, sensor_stride=sensor_stride
    )
    test_windows = FiveFrameSparseEvaluationDataset(
    ...
    print(f"\n{'=' * 60}")
    print(f"COMBINED RESULTS ({sample_count} predicted five-frame windows)")
    print(f"{'=' * 60}")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Evaluate conditional-DDPM reconstructions and the repository-"
            "consistent Kolmogorov residual of predicted five-frame windows."
        )
    )
    parser.add_argument(
        "--checkpoint", required=True, help="Trained .pt checkpoint"
    )
    parser.add_argument(
        "--data",
        default=(
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Batches per split; omit to evaluate every selected window",
    )
